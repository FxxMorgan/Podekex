...
/** Actualizar imagen del Pokémon y ajustar altura según las dimensiones variables del sprite */
...
/** Configurar información básica del Pokémon */
...
// Función auxiliar para obtener los tipos traducidos
function getTranslatedTypes(types) {
    var typeTranslations = {
        ...
        dragon: 'Dragón',
        dark: 'Siniestro',
        // Agrega más traducciones aquí para los demás tipos
    };

    var translatedTypes = [];
    for (var i = 0; i < types.length; i++) {
        var translatedType = typeTranslations[types[i]];
        if (translatedType) {
            translatedTypes.push(translatedType);
        } else {
            translatedTypes.push(types[i]);
        }
    }
...
/** Configurar estadísticas del Pokémon */
...
async function setupPokemonAbilities(pokemon) {
    document.getElementById('current-pokemon-ability-name-0').innerHTML = reemplazarTipoPokemon(dressUpPayloadValue(pokemon.abilities[0].ability.name));
    if (pokemon.abilities[1]) {
        document.getElementById('current-pokemon-ability-name-1').classList.remove('hide');
        document.getElementById('current-pokemon-ability-name-1').innerHTML = reemplazarTipoPokemon(dressUpPayloadValue(pokemon.abilities[1].ability.name));
    } else {
        document.getElementById('current-pokemon-ability-name-1').classList.add('hide');
    };
};


/** Configurar cadena de evolución (las 3 evoluciones) */
...
/** Configurar imágenes y nivel de evolución */
